...
import styles from "./PageSecondPlayer.module.css";
import PageArena from "../Arena/PageArena";
import useLang from "../../store/useLang";

const PageSecondPlayer = () => {
  ...
  function clickHandler(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
    ...
    settings.setName("second", nameSetInput.current.value);
    settings.setBuffer(0);
    settings.restoreRemainders();
    settings.setCurrentPage(<PageArena />);
  }
  return (
    <div className={styles.wrapper}>
      ...
      <Field>
        <CellsCreater player="second" clickHandler={cellClickHandler} />
      </Field>
    </div>
  );
};

export default PageSecondPlayer;
